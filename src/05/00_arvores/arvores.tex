...
\begin{frame}[fragile]{Uma hierarquia que você usa todo dia}
  \begin{minted}{text}
...
\begin{frame}{O tamanho do diretório não existe antes do conteúdo}
  ...
  \vspace{0.6cm}
  Ao final da aula essas duas observações voltam com nome: o \texttt{du}
...
\begin{frame}[fragile]{Você já percorreu árvores nesta disciplina}
  \textbf{Parser de expressões prefixadas}, na aula de recursão:
...
\begin{frame}{Duas consequências da definição}
  ...
  \begin{block}{A definição não distingue árvore de subárvore}
    Toda subárvore é uma árvore completa por direito próprio. Uma função que
...
func ArvoreExemplo() *No {
  ...
  \vspace{0.2cm}
  O recuo do código reproduz o desenho da árvore.
\end{frame}
...
func InOrdem(a *No) {
  ...
  binária. A próxima aula parte exatamente daí.
\end{frame}
...
\begin{frame}{Escolhendo o percurso pela tarefa}
  ...
  \vspace{0.3cm}
  A pós-ordem é o caso mais rígido: trocar a ordem das linhas não muda o
  resultado por gosto, \tcr{quebra a função}.
\end{frame}

\begin{frame}{Árvore de expressão}
  O parser da aula de recursão, agora com a árvore construída de fato:
  $2 \times (3 + 4)$.
...
\begin{frame}{Por que a recursão não resolve}
  \begin{block}{}
    A recursão dá acesso natural aos \textbf{filhos} de quem está sendo
